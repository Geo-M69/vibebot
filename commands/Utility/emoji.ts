import { SlashCommandBuilder, ChatInputCommandInteraction, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, StringSelectMenuBuilder, GuildEmoji, Message } from 'discord.js';
...
export default {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (query) {
            ...
            if (matches.length === 0) {
                return interaction.reply({ content: `No emojis found matching \`${query}\``, ephemeral: true });
            }
            ...
            const buildSelect = (p: number) => new StringSelectMenuBuilder()
            ...
            const rowButtons = new ActionRowBuilder<ButtonBuilder>().addComponents(prev, next);
            const rowSelect = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(buildSelect(0));
            ...
            collector.on('collect', async i => {
                try {
                    if (i.isStringSelectMenu()) {
                        ...
                        await i.update({ embeds: [previewEmbed], components: [new ActionRowBuilder<ButtonBuilder>().addComponents(openBtn, copyBtn, backBtn)] });
                        return;
                    }
                    ...
                    const newRowSelect = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(newSelect);
                    const newRowButtons = new ActionRowBuilder<ButtonBuilder>().addComponents(newPrev, newNext);
        ...
        const rowButtons = new ActionRowBuilder<ButtonBuilder>().addComponents(prevBtn, nextBtn);
        const rowSelect = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(buildSelect(0));
