...
exports.default = {
    ...
    async execute(interaction) {
        ...
        const member = interaction.guild ? interaction.guild.members.cache.get(target.id) || await interaction.guild.members.fetch(target.id).catch(() => null) : null;
        const embed = new discord_js_1.EmbedBuilder()
            .setTitle(`${target.tag} — User Info`)
            .setThumbnail(target.displayAvatarURL?.({ size: 256 }) || null)
            .setColor(0x0099ff)
            .addFields({ name: 'User ID', value: `${target.id}`, inline: true }, { name: 'Bot', value: `${target.bot ? 'Yes' : 'No'}`, inline: true }, { name: 'Account Created', value: formatTimestamp(target.createdTimestamp), inline: true });
        // Badges / flags
        try {
        ...
        if (member) {
            ...
            embed.addFields({ name: 'Server Join Date', value: formatTimestamp(member.joinedTimestamp), inline: true }, { name: 'Top Role', value: `${member.roles.highest?.name || 'None'}`, inline: true }, { name: 'Status', value: `${status}`, inline: true });
            ...
            const row = new discord_js_1.ActionRowBuilder().addComponents(prevButton, nextButton);
            // Send initial reply
            let reply;
            try {
            ...
            if (totalPages > 1) {
                ...
                const collector = reply.createMessageComponentCollector({ filter, componentType: discord_js_1.ComponentType.Button, time: 120000 });
                collector.on('collect', async (i) => {
                    ...
                    const newEmbed = discord_js_1.EmbedBuilder.from(embed);
                    ...
                    const newPrev = discord_js_1.ButtonBuilder.from(prevButton).setDisabled(page === 0);
                    const newNext = discord_js_1.ButtonBuilder.from(nextButton).setDisabled(page === totalPages - 1);
                    const newRow = new discord_js_1.ActionRowBuilder().addComponents(newPrev, newNext);
                    try {
                ...
                collector.on('end', async () => {
                    ...
                    try {
                        const disabledPrev = discord_js_1.ButtonBuilder.from(prevButton).setDisabled(true);
                        const disabledNext = discord_js_1.ButtonBuilder.from(nextButton).setDisabled(true);
                        const disabledRow = new discord_js_1.ActionRowBuilder().addComponents(disabledPrev, disabledNext);
