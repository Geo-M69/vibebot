...
const activeTimers = new Map(); // key: messageId -> { interval, end, remaining, paused, userId, channelId }
const userActiveCount = new Map(); // userId -> count
const channelActiveCount = new Map(); // channelId -> count
...
function getUpdateInterval(totalSeconds) {
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const controlRow = new ActionRowBuilder().addComponents(pauseBtn, resumeBtn, cancelBtn);
        ...
        collector.on('collect', async i => {
            try {
                if (i.customId === `cd_pause_${interaction.id}`) {
                    ...
                    await i.update({ components: [new ActionRowBuilder().addComponents(newPause, newResume, newCancel)] });
