                ...
                switch (_b.label) {
                    ...
                    case 1:
                        ...
                        if (!invoker || !invoker.permissions.has(discord_js_1.PermissionFlagsBits.BanMembers)) {
                            return [2 /*return*/, interaction.reply({ content: "You do not have permission to mute members.", ephemeral: true })];
                        }
                    ...
                    case 2:
                        ...
                        if (targetMember.id === interaction.user.id) {
                            return [2 /*return*/, interaction.reply({ content: "You can't mute yourself.", ephemeral: true })];
                        }
                        me = interaction.guild.members.me;
                        if (!me || !me.permissions.has(discord_js_1.PermissionFlagsBits.ModerateMembers)) {
                            return [2 /*return*/, interaction.reply({ content: "I don't have permission to mute members.", ephemeral: true })];
                        }
                        if (!targetMember.moderatable) {
                            return [2 /*return*/, interaction.reply({ content: "I cannot mute that user. They may have a higher role than me or special permissions.", ephemeral: true })];
                        }
                    ...
                    case 4:
                        ...
                        return [4 /*yield*/, targetUser.send("You have been muted in ".concat(interaction.guild.name, ".\nReason: ").concat(reason)).catch(function () { return null; })];
                    case 5:
                        // attempt to DM the user before muting (best-effort)
                        _b.sent();
                        // perform the mute
                        return [4 /*yield*/, targetMember.timeout(null, reason)];
                    case 6:
                        // perform the mute
                        _b.sent();
                        return [2 /*return*/, interaction.editReply({ content: "Muted ".concat(targetUser.tag, " (").concat(targetUser.id, ").\nReason: ").concat(reason) })];
                    case 7:
                        error_1 = _b.sent();
                        console.error("Error muting user:", error_1);
                        return [2 /*return*/, interaction.editReply({ content: "There was an error muting that user." })];
